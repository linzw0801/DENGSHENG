...
【版本历史】
   ...
   2026-08-06 v10: 新增风控条件④                   (原 fields 并排+缩写难读, 改为与邮件一致的全描述)
...
def theo_calc_from_data(ohlc):
    ...
    # 模拟 (含净值止损: 理论净值回撤15%清仓, 站上自身MA30恢复)
    ...
    cur = None
    # 净值止损状态
    NAV_STOP_DD = 0.15      # 净值回撤15%触发
    NAV_STOP_MA = 30        # 恢复用净值MA30
    peak = equity           # 理论净值历史高点
    in_cool = False         # 是否在冷静期(净值止损后)
    equity_hist = []        # 每日理论净值序列 (含当日)
    stop_events = []        # 止损/恢复事件
    for i in range(start_idx, n-1):
        ...
        risk_off = r1 or r2 or r3

        # 净值止损检查 (用昨日收盘后已确定的净值)
        if not in_cool and cur is not None and cur != 'REPO':
            if equity < peak*(1 - NAV_STOP_DD):
                # 触发: 清仓转逆回购, 进入冷静期
                in_cool = True
        ...
        # 冷静期恢复检查: 净值站上自身MA30
        if in_cool:
            ma30 = sum(equity_hist[-NAV_STOP_MA:])/NAV_STOP_MA if len(equity_hist) >= NAV_STOP_MA else None
            if ma30 is not None and equity >= ma30:
                in_cool = False
                stop_events.append(('恢复', dates[i], equity/peak - 1))

        if risk_off or in_cool:
