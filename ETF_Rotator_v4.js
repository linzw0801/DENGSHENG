// ETF轮动 最终版 v4 (Scriptable, iOS) — 修复 setTimeout / 自发邮件
...
// ★ 修复：用 Scriptable 原生 Timer.wait()，不用 setTimeout
const sleep=(sec)=>{if(typeof Timer!=="undefined"&&typeof Timer.wait==="function")return Timer.wait(sec);return new Promise(r=>setTimeout(r,sec*1000));};
...
try{
  ...
  if(data.error){
    ...
    if(typeof Script!=="undefined"&&Script.setShortcutOutput) Script.setShortcutOutput(reportHtml);
    console.log(reportHtml);
    try{ await sendMail(subject, reportHtml); }catch(mailErr){ console.log("⚠️ 邮件发送失败: "+String(mailErr.message||mailErr)); }
  }
}catch(err){
